# Asana utilities
#
# Initially pulled from https://github.com/pandemicsyn/asana
...
class AsanaAPI(object):
    ...
    def __init__(self, debug=False, config_file=CONFIG_FILE, asana_api_key=None):
        if config_file:
            config = ConfigParser.ConfigParser()
            config.read(os.path.expanduser(config_file))
            self.apikey = config.get("Asana", "asana_api_key")
            self.workspace = int(config.get("Asana", "workspace"))
        elif asana_api_key:
            self.apikey = asana_api_key
        else:
            print("no config file or api key - i need these things, dawg!")
    ...
    def list_users(self, filters=None):
        ...
        """
        if self.workspace:
            return self._asana('workspaces/%s/users' % self.workspace)
        else:
            if filters:
    ...
    def list_tasks(self, assignee, include_archived=False,
        ...
        target = "tasks?workspace=%d&assignee=%s&include_archived=%s" % (
            self.workspace, assignee, include_archived)
    ...
    @retry(stop_max_attempt_number=ASANA_MAX_TRIES)
    ...
    def list_projects(self, include_archived=True):
        ...
        if self.workspace:
            target = "workspaces/%d/" % (self.workspace) + target
    ...
    @retry(stop_max_attempt_number=ASANA_MAX_TRIES)
    def get_project_tasks(self, project_id, include_archived=False, modified_since=None):
        """Get project tasks
        ...
        if include_archived:
            ...
            include_archived = "false"
        target = "projects/%d/tasks?include_archived=%s" % (project_id, include_archived)
    ...
    def create_task(self, name, assignee=None, assignee_status=None,
        ...
        """
        payload = {'name': name, 'workspace': self.workspace}
        if assignee:
    ...
    def create_project(self, name, team=None,
        ...
        """
        payload = {'name': name, 'workspace': self.workspace, 'team': team}
        if notes:
            payload['notes'] = notes
        if archived:
    ...
    def get_tags(self):
        ...
        """
        return self._asana('workspaces/%d/tags' % self.workspace)
    ...
    def create_tag(self, tag):
        ...
        """
        payload = {'name': tag, 'workspace': self.workspace}
